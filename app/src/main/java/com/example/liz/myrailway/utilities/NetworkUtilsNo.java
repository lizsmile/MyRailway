...
public class NetworkUtilsNo {
    private final static String APPKEY = "a0860d470a2a1cede6b613b121959986";

    final static String url = "http://apis.juhe.cn/train/s";//请求接口地址
    final static String method = "Get";


    public static URL buildUrl(String trainNo) {
        String strUrl;
        URL queryUrl = null;
        HashMap<String, String> params = new HashMap<>();//请求参数
        params.put("name", trainNo);//列车车次
    ...
    public static String getJsonResultNo(URL url) throws IOException {
        String result = null;
        try {
            result = net(url);
            Log.v("function","get json result no");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
